...
exports('BankTruck', BankTruck)

NewAlert = function(DispatchData)
    TriggerServerEvent('mw-dispatch:server:NewAlert', DispatchData)
end

updateCharInfo = function(callsign, firstname, lastname, rank)
    SendNUIMessage({
        type = 'CHARACTER_INFO',
        data = {
            callsign = callsign,
...
local function Carchop(camId)
    local currentPos = GetEntityCoords(PlayerPedId())
    local locationInfo = getStreetandZone(currentPos)
    local gender = GetPedGender()
    TriggerServerEvent("dispatch:server:notify", {
        dispatchcodename = "Car Chop", -- has to match the codes in sv_dispatchcodes.lua so that it generates the right blip
        dispatchCode = "10-90",
        firstStreet = locationInfo,
        gender = gender,
        camId = camId,
