...
--         exports['qb-target']:AddBoxZone(Store['Type'] .. i, Store['Coords'], Store['Length'], Store['Width'], {
--             name = Store['Type'] .. i,
--             debugPoly = false,
...
--                 TriggerEvent('qb-clothing/client/getOutfits', Room['Job'], Grade)
--             end
--         else
--             TargetAction = function()
...
--         exports['qb-target']:AddBoxZone('clothing_' .. Room['Job'] .. i, Room['Coords'], Room['Length'], Room['Width'],
--             {
...
function PolyClothing()
    ...
    RoomCombo:onPlayerInOut(function(isPointInside, point, zone)
        if isPointInside then
            ...
            local JobName = Config.ClothingRooms[ZoneId]['IsGang'] and PlayerData.gang.name or PlayerData.job.name
...
function SaveSkin(IsAdmin)
    QBCore.Functions.TriggerCallback('qb-clothing/server/pay-for-clothes', function(HasPaid)
        if HasPaid then
            local Model = GetEntityModel(PlayerPedId())
            local Clothing = json.encode(Config.SkinData)
            TriggerServerEvent("qb-clothing/saveSkin", Model, Clothing, IsAdmin)
        end
    end, IsAdmin)
end
