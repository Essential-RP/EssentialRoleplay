        ...
        if LocalPlayer.state.isLoggedIn then
            ...
            if IsPedInAnyVehicle(ped, false) and not IsHotwiring then
                ...
                if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() and not HasKeys(plate) and not isBlacklistedVehicle(vehicle) then
                    sleep = 5
                             if not ShownDrawText then
                        ShownDrawText = true
                        exports['qb-core']:DrawText('[H] Search', 'left')
                    end
                    ...
                    if IsControlJustPressed(0, 74) then
                        ...
                        -- TriggerEvent("plzwaitbro")
                    end
                else
                    if ShownDrawText then
                        ShownDrawText = false
                        exports['qb-core']:HideText()
                    end
                end
...
RegisterNetEvent('lockpicks:UseLockpick2', function(isAdvanced)
    LockpickDoor(isAdvanced)
end)

RegisterNetEvent('lockpicks:UseLockpick', function(isnotAdvanced)
    LockpickDoor1(isnotAdvanced)
end)
...
function LockpickDoor1(isnotAdvanced)
    local ped = PlayerPedId()
    ...
    exports['ps-ui']:Circle(function(success)
        if success then
            ...
            exports['ps-ui']:Notify("You failed lock-pick the vehicle !", "error")
        end
    ...
    QBCore.Functions.Progressbar("rob_keys", Lang:t("progress.acjack"), Config.CarjackingTime, false, true, {}, {}, {}, {}, function()
        ...
        if hasWeapon and isCarjacking then
            if math.random() <= Config.CarjackChance[tostring(GetWeapontypeGroup(weaponHash))] then
                ...
                exports['ps-ui']:Notify('The driver fled and did not value his life', 'error')
                MakePedFlee(target)
                TriggerServerEvent('hud:server:GainStress', math.random(1, 4))
            end
            isCarjacking = false
