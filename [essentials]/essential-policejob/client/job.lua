...
local function SetWeaponSeries2()
    for k, _ in pairs(Config.smallarmory.items) do
        if k < 6 then
            Config.smallarmory.items[k].info.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(1) .. QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(4))
        end
    end
end
...
RegisterNetEvent('police:client:showFingerprintId', function(fid, name)
    SendNUIMessage({
...
RegisterNetEvent('qb-police:client:openSmallArmory', function()
    local authorizedItems = {
    ...
    local index = 1
    for _, armoryItem in pairs(Config.smallarmory.items) do
        for i=1, #armoryItem.authorizedJobGrades do
            if armoryItem.authorizedJobGrades[i] == PlayerJob.grade.level then
                authorizedItems.items[index] = armoryItem
                authorizedItems.items[index].slot = index
...
RegisterNetEvent('qb-police:client:openAmmo', function()
    local authorizedItems = {
        label = Lang:t('menu.pol_armory3'),
        slots = 30,
        items = {}
    }
    local index = 1
    for _, armoryItem in pairs(Config.Ammo.items) do
        for i=1, #armoryItem.authorizedJobGrades do
            if armoryItem.authorizedJobGrades[i] == PlayerJob.grade.level then
                authorizedItems.items[index] = armoryItem
                authorizedItems.items[index].slot = index
