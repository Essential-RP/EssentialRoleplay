...
if Config.Framework == 'QBCore' then
    exports['qb-target']:AddTargetModel(models, {
        options = {
            {
            type = "client",
            event = "essential-atmrobbery:robatm",
            icon = 'fa-sharp fa-solid fa-arrow-up-from-bracket',
            label = 'Rob ATM',
            item = {Config.HackItem.fp, Config.HackItem.sim}
            }
        },
      distance = 2.5,
    })
...
function hacksuccess()
    ...
    TriggerServerEvent('essential-atmrobbery:server:timer')

    if Config.MoneyType == true then

	    TriggerServerEvent("essential-atmrobbery:server:success")

    else

        TriggerServerEvent("essential-atmrobbery:server:success2")
...
---------------------------------------------------------------------------------------# Events #---------------------------------------------------------------------------------------
RegisterNetEvent('essential-atmrobbery:robatm', function()
    if Config.Framework == 'QBCore' or 'OX' then
        
        if CurrentCops >= Config.Cops then

            QBCore.Functions.TriggerCallback("essential-atmrobbery:server:Cooldown",function(isCooldown)
                if not isCooldown then

                    TriggerEvent('animations:client:EmoteCommandStart', {"parkingmeter"})
                    TriggerServerEvent('essential-atmrobbery:server:removeitem')
                ...
                QBCore.Functions.Notify("Looks like an ATM in the area has been hit wait a bit and come back")
